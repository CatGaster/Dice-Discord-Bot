...
(Если вы не нуждаетесь в использовании языковой модели то оставьте OPENROUTER_API_KEY= без ключа)
...
3. Создайте файл `.env` и добавьте в него токен вашего бота:
   ...
   OPENROUTER_API_KEY=<ваш ключ API OpenRouter> (если не используется оставьте пустым)
   ```
